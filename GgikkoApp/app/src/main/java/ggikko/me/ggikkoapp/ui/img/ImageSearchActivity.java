...
public class ImageSearchActivity extends InjectionActivity {
  ...
  Toolbar activity_image_toolbar;
  @BindView(R.id.activity_image_tabs)
  TabLayout activity_image_tabs;
  @BindString(R.string.app_name)
  String app_name;
  @BindString(R.string.searchview_hint)
  String searchview_hint;
  ...
  /**
   * toolbar 타이틀 적용 support actionbar에 toolbar등록
   */
  ...
  /**
   * 1. view pager를 등록 2. section pager adapter를 생성하고 각 page activity정함 3. 페이지 변경이 일어날 때마다 archive
   * fragment의 content refresh 시켜줌 4. 탭을 이동할 때 아이콘에 Alpha값을 주어 client가 현재 어떤 페이지에 있는지 인지시켜줌
   */
  ...
  /**
   * 커스텀 탭을 이용하여 alpha값을 줌 - 100 setupTabs method에서 사용
   */
  ...
  /**
   * 탭의 커스텀 아이콘의 alpha값을 줍니다 - 200 setupTabs method에서 사용
   */
  ...
  /**
   * 검색 View 생성 및 힌트 설정 완료 후에 softkeyboard 내리고 clear focus //TODO 추후에 publish subscriber를 이용하여 일정 시간
   * 단위 후 입력이 안되면 자동으로 service를 call하는 방식 적용 예정
   */
  @Override
